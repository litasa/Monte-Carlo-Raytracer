...
#include "Renderer.h"
Camera::Camera(glm::vec3 origin, glm::vec3 look_at, glm::vec3 world_up, float vertical_fov, float near_plane_distance, unsigned int rays_per_pixel, unsigned int shadow_rays, unsigned int maximum_recursion_depth)
	: _origin(origin), _rays_per_pixel(rays_per_pixel), _near_plane_distance(near_plane_distance), _vertical_fov(vertical_fov), _shadow_rays(shadow_rays), _max_recursion_depth(maximum_recursion_depth) {
...
void Camera::concurrent_help_func(const Scene &scene, PixelBuffer &buffer, unsigned int x_id, unsigned y_id, unsigned int tile_width, unsigned int tile_height) {
	//Lock so we don't use use same random sample for each thread
	mutex.lock();
	std::srand((unsigned int)std::time(0));
	mutex.unlock();

	Ray ray;
	Renderer renderer(scene, _max_recursion_depth, _shadow_rays);
	...
	for (unsigned int y = y_id * tile_height; y < (y_id + 1) * tile_height; ++y) {
		for (unsigned int x = x_id * tile_width; x < (x_id + 1) * tile_width; ++x) {
			...
			//Different ways if we use supersampling and jittering or not
			...
			mutex.lock();
			buffer.set_rgb_value(x, y, final_color);
			mutex.unlock();
		}
	}
